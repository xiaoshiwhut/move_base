...
def pose_callback(msg):
    ...
    if msg.status.status == 3 and count>0 :  #成功到达任意目标点，前往下一目标点
        ...
        if index == count:                   #当index等于count时，表示所有目标点完成，重新开始巡航
            ...
            # #机器人执行动作处代码（单次任务）
            # while True:
            #     print("到达最后一点,执行等待30秒动作")
            #     time.sleep(10)  # 每秒打印一次

            #     if time.time() - start_time > 30:  # 如果已经超过2分钟
            #         # print("前往 %d 点" %index)
            ...
            #机器人执行动作处代码（循环任务）
            while True:
                print("到达最后一点,执行等待10秒动作")
                time.sleep(10)  # 每秒打印一次

                if time.time() - start_time > 10:  # 如果已经超过2分钟
                    print("前往 %d 点" %index)
                    index += 1 #下一次要发布的目标点序号
                    break
            #发布下一导航点的数据
            ...
            pose.pose.orientation.w = markerArray.markers[index].pose.orientation.w
            while True:
                print("到达 %d 点,执行等待10秒动作" %(index-1))
                time.sleep(10)  # 每秒打印一次
    ...
    #     if time.time() - start_time > 60:  # 如果已经超过2分钟
    #         index += 1 #下一次要发布的目标点序号
    #         break
...
if __name__ == '__main__':
    ...
    send_mark()  #正常导航2D nva Goal话题/move_base_simple/goal  多点导航/send_mark_goal
